...
var lastshow = null;
var show_changed = false;
function update_np(force) {
    var banner = false;
    if ($("div.nowplaying-banner").length > 0) {
        banner = true;
    }
    $.getJSON('/api/site/nowplaying?full='+banner,function(data) {
        if (data.success) {
            var disco = $("div.disco");
            var background = disco.find('div.background');
            if (data.data.show) {
                if (data.data.show.id != lastshow) {
                    disco.find('.hp').show();
                    dj = disco.find('.dj').show();
                    dj.find('img').attr('src','/static/img/cb/'+data.data.show.user.countryball);
                    if (data.data.show.logo) {
                        background.css('background-image', "url('"+data.data.show.logo+"')");
                    } else {
                        background.css('background-image', "");
                    }
                    if (data.data.show.name) {
                        disco.find('#ticker').html(data.data.show.name);
                    }
                    else {
                        disco.find('#ticker').html('DJ did not enter a name へ‿(ツ)‿ㄏ');
                    }
                    lastshow = data.data.show.id;
                }
            } else {
